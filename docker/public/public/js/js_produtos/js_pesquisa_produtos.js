	    ...
	    function busca(volta_primeira_pagina=0){ 
	        ...
	        $.ajax({    
	                ...
	                success:function(results){
				        ...
				        $.each(results.lista, function() {
                            ...
                            if(this.status==0){
                            var excluir = "<a href='#' alt='Excluir Produto' onclick='exclui_produto("+this.produto_id+")'  ><i class='espaco_icone_acao fa fa-trash-o'></i></a>";
                            }else{
                                var excluir = "";
                            }
				            ...
				            $("#listagem_dados").append("<tr>"+
				                                            "<td>"+excluir+"<a href='./alterar_produtos?id_registro="+this.produto_id+"' ><i class='espaco_icone_acao fa fa-pencil'></i></a>"+
				                                            "</td>"+
				                                            "<td>"+this.nome+"</td>"+
            ...
            function exclui_produto(id_produto){
                 ...
                 // abre a confirmacao para cancelar a PV
                    if (confirm("Deseja confirmar a exclusão do produto ?")) {
                      envia_exclusao(id_produto)
                    }
   ...
   /**********Funcao que envia o post para o controller que informa PEP************/
      function envia_exclusao(id_produto){
        ...
        $.ajax({
                type: 'DELETE',  
                url: './api/v1/produtos/'+id_produto,
